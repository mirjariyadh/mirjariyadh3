...
function generateSmartFallbackResponse(userMessage: string): { reply: string; suggestions: string[] } {
  ...
  if (query.includes("point cloud") || query.includes("scan to bim") || query.includes("laser scan") || query.includes("scan")) {
    return {
  ...
  if (query.includes("mep") || query.includes("hvac") || query.includes("plumbing") || query.includes("electrical") || query.includes("clash")) {
    return {
  ...
  if (query.includes("cost") || query.includes("price") || query.includes("pricing") || query.includes("rate") || query.includes("কত টাকা") || query.includes("খরচ")) {
    return {
      ...
      suggestions: ["Request a Quote", "Prepare Project Requirements", "View Projects", "Contact Mirja"]
    };
  }

  if (query.includes("architecture") || query.includes("architectural") || query.includes("revit") || query.includes("cad to revit") || query.includes("2d to 3d")) {
    return {
  ...
  if (query.includes("quote") || query.includes("hire") || query.includes("contact") || query.includes("যোগাযোগ") || query.includes("inquiry")) {
    return {
  ...
  // Default welcome/helpful fallback
  return {
    reply: `I can help you explore Mirja Riyadh's BIM services, inspect verified portfolio projects (Architecture, MEP, Scan-to-BIM, AutoCAD), explain workflows, or help you prepare a project inquiry.\n\nWhat type of project are you currently planning?`,
  ...
  try {
    ...
    if (ai) {
      try {
        ...
        const replyText = response.text || "I'm ready to assist with your BIM and Revit modeling requirements.";

        // Generate contextual quick action chips based on content
        const suggestions: string[] = [];
        const lowerReply = replyText.toLowerCase();
        const lowerUser = latestUserMsg.toLowerCase();

        if (lowerReply.includes("point cloud") || lowerUser.includes("scan") || lowerUser.includes("point cloud")) {
          suggestions.push("View Point Cloud Projects", "Scan-to-BIM Workflow");
        } else if (lowerReply.includes("mep") || lowerUser.includes("mep") || lowerUser.includes("hvac") || lowerUser.includes("clash")) {
          suggestions.push("View MEP Projects", "Clash Detection");
        } else if (lowerReply.includes("architecture") || lowerUser.includes("architecture") || lowerUser.includes("cad")) {
    ...
    res.status(500).json({
      reply: "Sorry, I couldn't process that request right now. You can still explore the portfolio or send a direct project inquiry.",
