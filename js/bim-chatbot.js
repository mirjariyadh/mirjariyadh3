...
(function () {
  ...
  function generateClientSmartFallback(userText) {
    ...
    if (q.includes('point cloud') || q.includes('scan to bim') || q.includes('laser scan') || q.includes('scan')) {
      return {
        reply: `Point Cloud to BIM (Scan-to-BIM) converts 3D laser-scan data into high-precision Autodesk Revit models (LOD 200–350).\n\nTypical Workflow:\nPoint Cloud → Revit Modeling → Model Review → Documentation\n\nWould you like to explore our Scan-to-BIM project portfolio?`,
        suggestions: ['View Point Cloud Projects', 'Request a Quote', 'Explore Services']
      };
    }

    if (q.includes('mep') || q.includes('hvac') || q.includes('plumbing') || q.includes('electrical') || q.includes('clash')) {
      return {
        ...
        suggestions: ['View MEP Projects', 'Request a Quote', 'Explore Services']
      };
    }

    if (q.includes('price') || q.includes('cost') || q.includes('pricing') || q.includes('rate') || q.includes('খরচ') || q.includes('কত')) {
      return {
        reply: `Project pricing depends on factors such as building size, scope of disciplines, source drawings (CAD/PDF/Point Cloud), required LOD, and timeline.\n\nI can help you prepare the project information needed for an accurate quote.`,
        suggestions: ['Request a Quote', 'View Projects', 'Explore Services']
      };
    }

    if (q.includes('quote') || q.includes('hire') || q.includes('contact') || q.includes('যোগাযোগ')) {
      return {
